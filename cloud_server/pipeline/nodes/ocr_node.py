...
class PlateRecognitionNode(PipelineNode):
    def __init__(self):
        ...
        self._frame_count = 0
        self._plate_cache = {}  # {device_id: {track_id: (plate_number, confidence)}}

    def load_model(self):
        logger.info("[PlateOCR] Node ready (function-call mode)")
        self._frame_count = 0
    ...
    def _do_process(self, context: FrameContext) -> FrameContext:
        ...
        for idx, box in enumerate(boxes):
            ...
            try:
                ...
                logger.error("[PlateOCR] recognize_plate() 尚未实现！等待模型同学实现。")
                plates.append("")
                confidences.append(0.0)
        ...
        if self._frame_count <= 5 or self._frame_count % 30 == 0:
            logger.info(
                ...
                f"{recognized} new plates recognized, {cached} plates from cache"
            )
